...
|---|---|
| `*.html` (31 pages) | **Source.** Edit directly — except the blocks between `<!-- BUILD:… -->` markers. |
...
| `data/ministries.json` | **Source of truth** for the ministries — the grid on `ministries.html` and the body of each `ministry-*.html` page. Rendered by `npm run ministries`. |
| `MINISTRIES-FOR-THE-PARISH.md` | **Generated** by the same build: what each ministry still has to tell us. |
...
The **only** generated folder is `dist/chunks/`. `npm run dev` keeps it in
step automatically; `npm run lint` fails if it has drifted. Everything else a
tool writes is the markup between a pair of `<!-- BUILD:… -->` markers, plus
the three generated `.md` ask-lists above.
...
npm run audit:a11y     # axe-core, WCAG 2.1 AA, 31 pages x 2 viewports
...
There are 88 `<!-- TODO: verify -->` comments in the pages. They are
...
`ntgocSiteFooter`) are the same markup on all thirty-one pages — about a third of
all the HTML here. Edit them **once, in one page**, then propagate:

```sh
npm run shell                      # from index.html to the other thirty
...
above. Do not make the change thirty-one times by hand; `npm run shell -- --check`
