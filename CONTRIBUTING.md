...
|---|---|
| `*.html` (18 pages) | **Source.** Edit directly — except the blocks between `<!-- BUILD:… -->` markers. |
...
npm run audit:a11y     # axe-core, WCAG 2.1 AA, 18 pages x 2 viewports
...
`ntgocSiteFooter`) are the same markup on all eighteen pages — about a third of
...
seventeen pages agree and one does not.
...
above. Do not make the change eighteen times by hand; `npm run shell -- --check`
